...
class Example:
    def __init__(
        ...
        solver_type: str | None = "featherstone",
        headless: bool = False,
    ):
        ...
        # NOTE: (WH) better parameter with less jitering artifacts
        self.num_substeps = 8
        self.iterations = 9
        ...
        # NOTE: (WH) Choose SolverFeatherstone or SolverMujoco
        if solver_type == "mujoco":
            print("Using the Mujoco solver for rigid bodies.")
            self.robot_solver = newton.solvers.SolverMuJoCo(self.model)
        elif solver_type == "featherstone":
            print("Using the Featherstone solver for rigid bodies.")
            self.robot_solver = SolverFeatherstone(self.model, update_mass_matrix_interval=self.num_substeps)
        else:
            print(f"Unsupported rigid solver: {solver_type}, falling back to the Featherstone solver")
            self.robot_solver = SolverFeatherstone(self.model, update_mass_matrix_interval=self.num_substeps)
        ...
        # TODO: (WH) The viewer API below is deprecated. Use newton.viewer.ViewerGL and newton.viewer.ViewerUSD in future
        if not headless:
            ...
            print("Using the interactive viewer.")
        elif stage_path:
            self.renderer = newton.viewer.RendererUsd(self.model, stage_path)
            print("Using the USD viewer. Simulation results will be saved to", stage_path)
        else:
            self.renderer = None
    ...
    def create_articulation(self, builder):
        ...
        # NOTE: (WH) To prevent the robot from colliding with the table at initialization, slightly increase the y offset. Set offset_y=0.0 to disable this
        offset_y = 0.0
        # offset_y = 0.005
        ...
        self.robot_key_poses = np.array(
            [
                ...
                # # top left
                [3, 0.31, 0.12+offset_y, 0.60, *vec_rotation(0.0, -1.0, 0.0), clamp_open_activation_val],
                [2, 0.31, 0.12+offset_y, 0.60, *vec_rotation(0.0, -1.0, 0.0), clamp_close_activation_val],
                ...
                [3, 0.15, 0.10+offset_y, 0.33, *vec_rotation(0.0, -1.0, 0.0), clamp_open_activation_val],
                [3, 0.15, 0.10+offset_y, 0.33, *vec_rotation(0.0, -1.0, 0.0), clamp_close_activation_val],
                ...
                [2, -0.28, 0.10+offset_y, 0.60, *vec_rotation(0.0, -1.0, 0.0), clamp_open_activation_val],
                [2, -0.28, 0.10+offset_y, 0.60, *vec_rotation(0.0, -1.0, 0.0), clamp_close_activation_val],
                ...
                # # bottom left
                [3, -0.18, 0.105+offset_y, 0.30, *vec_rotation(0.0, -1.0, 0.0), clamp_open_activation_val],
                [3, -0.18, 0.10+offset_y, 0.30, *vec_rotation(0.0, -1.0, 0.0), clamp_close_activation_val],
                ...
                [2, -0.0, 0.092+offset_y, 0.21, *vec_rotation(0.0, -1.0, 0.0), clamp_open_activation_val],
                [2, -0.0, 0.092+offset_y, 0.21, *vec_rotation(0.0, -1.0, 0.0), clamp_close_activation_val],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to the output USD file. Only used if running in headless mode.",
    )
    parser.add_argument(
        "--headless",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run the simulation without opening an interactive window. When enabled, the animation is rendered off-screen and saved to the specified USD file. You can use Blender to open the USD file and play the animation.",
    )
    parser.add_argument(
        "--num-frames",
    ...
    parser.add_argument(
        ...
        help="Choose the rigid body solver: \"featherstone\" or \"mujoco\".",
    )

    args = parser.parse_known_args()[0]

    with wp.ScopedDevice(args.device):
        example = Example(stage_path=args.stage_path, num_frames=args.num_frames, solver_type=args.solver, headless=args.headless)
        ...
        # if using USD renderer, save the result into the usd file
        if example.renderer and args.headless:
