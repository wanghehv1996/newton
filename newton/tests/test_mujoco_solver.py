...
class TestMuJoCoSolver(unittest.TestCase):
    ...
    def test_randomize_joint_q(self):
        ...
        for env_idx in range(num_envs):
            ...
            full_joint_q_numpy_modified[q_start_offset_for_current_env + 0 : q_start_offset_for_current_env + 3] += (
                current_delta
            )
            # Generate and normalize random quaternion
            raw_q0 = rng.random(4) * 2.0 - 1.0  # Random numbers in [-1, 1]
            q_tmp_0 = wp.normalize(wp.quat(raw_q0[0], raw_q0[1], raw_q0[2], raw_q0[3]))
            full_joint_q_numpy_modified[q_start_offset_for_current_env + 3 : q_start_offset_for_current_env + 7] = [
                q_tmp_0[0],
                q_tmp_0[1],
                q_tmp_0[2],
                q_tmp_0[3],
            ...
            full_joint_q_numpy_modified[q_start_offset_for_current_env + 7 : q_start_offset_for_current_env + 10] += (
                current_delta
            )
            # Generate and normalize random quaternion
            raw_q1 = rng.random(4) * 2.0 - 1.0  # Random numbers in [-1, 1]
            q_tmp_1 = wp.normalize(wp.quat(raw_q1[0], raw_q1[1], raw_q1[2], raw_q1[3]))
            full_joint_q_numpy_modified[q_start_offset_for_current_env + 10 : q_start_offset_for_current_env + 14] = [
                q_tmp_1[0],
                q_tmp_1[1],
                q_tmp_1[2],
                q_tmp_1[3],
        ...
        wp.launch(
            ...
            inputs=[
                ...
                solver.mjw_data.qvel,  # MuJoCo qvel for all envs - just to make the kernel happy
                joints_per_env,
                self.model.up_axis,
