...
def evaluate_stvk_force_hessian(
    ...
    if trace_G < 0.0:
        # 当布料被压缩时，将 lambda 降低到原来的 1% (或者更小)。
        # 这允许布料塌陷并形成细碎的褶皱，而不是像橡胶一样抵抗压缩。
        effective_lmbd = lmbd * 0.001         
        # 可选：稍微降低 mu (剪切刚度)，让压缩时的形状更容易改变
        effective_mu = mu * 0.5 

    # First Piola-Kirchhoff stress tensor (StVK model)
    # PK1 = 2*mu*F*G + lambda*trace(G)*F = [PK1_col0, PK1_col1] (3x2)
    lambda_trace_G = effective_lmbd * trace_G
    two_mu = 2.0 * effective_mu
    ...
    two_dpsi_dIc = -effective_mu + (0.5 * Ic - 1.0) * effective_lmbd
    ...
    H_IIc00_scaled = effective_mu * (f0_dot_f0 * I33 + 2.0 * f0_outer_f0 + f1_outer_f1)
    H_IIc11_scaled = effective_mu * (f1_dot_f1 * I33 + 2.0 * f1_outer_f1 + f0_outer_f0)
    H_IIc01_scaled = effective_mu * (f0_dot_f1 * I33 + f1_outer_f0)

    # d2(psi)/dF^2 components
    d2E_dF2_00 = effective_lmbd * f0_outer_f0 + two_dpsi_dIc * I33 + H_IIc00_scaled
    d2E_dF2_01 = effective_lmbd * f0_outer_f1 + H_IIc01_scaled
    d2E_dF2_11 = effective_lmbd * f1_outer_f1 + two_dpsi_dIc * I33 + H_IIc11_scaled
    ...
    if damping > 0.0:
        ...
        kd_lmbd = effective_lmbd * damping
...
def evaluate_dihedral_angle_based_bending_force_hessian(
    ...
    # ----------------------------------------------------------
    # [Core Modification Start]: Saturation / Yielding Bending
    # ----------------------------------------------------------
    
    rest_angle = edge_rest_angle[bending_index]
    angle_diff = theta - rest_angle
    
    # 基础刚度乘上边长（面积权重）
    k_base = stiffness * edge_rest_length[bending_index]

    # 【修改点】：使用 tanh 进行力矩饱和
    # 原始逻辑: dE_dtheta = k * angle_diff  (线性弹簧，折得越深回弹越大 -> 橡胶感)
    # 新逻辑:   dE_dtheta = k * tanh(angle_diff) (饱和弹簧 -> 棉布感)
    # 
    # 物理意义：当折叠角度变大时，恢复力不再无限增加。
    # 这模拟了织物纤维的“屈服”，允许形成较深的折痕而不反弹。
    
    # 缩放因子控制饱和的快慢。
    # 2.0 表示大约在 0.5 弧度(30度)左右就开始进入软化区域。
    scale_factor = 2.0 
    tanh_val = wp.tanh(angle_diff * scale_factor)
    
    # 计算力 (一阶导)
    # 有效刚度随着角度增加而降低
    dE_dtheta = k_base * tanh_val / scale_factor 

    # 计算 Hessian (二阶导)
    # d(tanh(x))/dx = 1 - tanh^2(x) = sech^2(x)
    # 当角度很大时，Hessian 会变得很小，这意味着系统不会试图强硬地矫正大角度弯曲。
    sech_sq = 1.0 - tanh_val * tanh_val
    k_hessian = k_base * sech_sq 
    ...
    # 使用修改后的 dE_dtheta 和 k_hessian
    bending_force = -dE_dtheta * dtheta_dx
    bending_hessian = k_hessian * wp.outer(dtheta_dx, dtheta_dx)
    ...
    if damping > 0.0:
        ...
        # 【阻尼修改建议】
        # 即便弯曲刚度因为 saturation 变小了，阻尼依然要保持强劲。
        # 棉布是高耗能的（不抖动）。所以这里我们用 k_base 而不是减弱后的 k_hessian。
        damping_coeff = damping * k_base 
...
@wp.kernel
def forward_step_penetration_free_with_warm_start(
    dt: float,
    gravity: wp.array(dtype=wp.vec3),
    pos_prev: wp.array(dtype=wp.vec3),
    pos: wp.array(dtype=wp.vec3),
    ...
    particle_conservative_bounds: wp.array(dtype=float),
    inertia: wp.array(dtype=wp.vec3),
    warm_start_displacement: wp.array(dtype=wp.vec3),
):
    """
    Forward integration with penetration-free conservative bounds and warm start.
    
    The warm start displacement modifies the starting position for VBD iterations,
    but does NOT change the inertia target. This provides a better initial guess
    while maintaining the physically correct target.
    
    - inertia: remains as pure explicit integration result (physics target)
    - pos: modified by warm_start_displacement (better starting point)
    - VBD iterations will balance between the warm-started pos and the physics target
    """
    particle_index = wp.tid()
    ...
    # Explicit integration - this is the physics target
    vel_new = vel[particle_index] + (gravity[0] + external_force[particle_index] * inv_mass[particle_index]) * dt
    pos_inertia = pos[particle_index] + vel_new * dt
    inertia[particle_index] = pos_inertia
    
    # # Apply warm start displacement to starting position only
    warm_disp = warm_start_displacement[particle_index]
    if wp.length(warm_disp) > 1e-5:
        pos_inertia = pos[particle_index] + warm_disp
        inertia[particle_index] = pos[particle_index] + warm_disp
        pos[particle_index] = pos_inertia
        # pos[particle_index] = apply_conservative_bound_truncation_new(
        #     particle_index, pos_inertia, pos_prev_collision_detection, particle_conservative_bounds, rate=1.0
        # )
    else:
        # # Apply conservative bound truncation to prevent penetration
        pos[particle_index] = apply_conservative_bound_truncation_new(
            particle_index, pos_inertia, pos_prev_collision_detection, particle_conservative_bounds, rate=1.0
        )
...
@wp.func
def apply_conservative_bound_truncation_new(
    v_index: wp.int32,
    pos_new: wp.vec3,
    pos_prev_collision_detection: wp.array(dtype=wp.vec3),
    particle_conservative_bounds: wp.array(dtype=float),
    ...
    particle_pos_prev_collision_detection = pos_prev_collision_detection[v_index]
    accumulated_displacement = pos_new - particle_pos_prev_collision_detection
    conservative_bound = rate * particle_conservative_bounds[v_index]
...
class SolverVBDNew(SolverBase):
    ...
    @override
    def step(self, state_in: State, state_out: State, control: Control, contacts: Contacts, dt: float, warm_start_displacement=None):
        if self.handle_self_contact:
            self.simulate_one_step_with_collisions_penetration_free(state_in, state_out, control, contacts, dt, warm_start_displacement)
        else:
            self.simulate_one_step_no_self_contact(state_in, state_out, control, contacts, dt, warm_start_displacement)

    def simulate_one_step_no_self_contact(
        self, state_in: State, state_out: State, control: Control, contacts: Contacts, dt: float, warm_start_displacement=None
    ):
        model = self.model
    ...
    def simulate_one_step_with_collisions_penetration_free(
        ...
        # Use warm start kernel if displacement is provided, otherwise use regular kernel
        if warm_start_displacement is not None:
            wp.launch(
                kernel=forward_step_penetration_free_with_warm_start,
                inputs=[
                    dt,
                    model.gravity,
                    self.particle_q_prev,
